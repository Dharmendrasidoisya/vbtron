...
namespace Botble\Solution\Listeners;
...
class RenderingSiteMapListener
{
    public function handle(RenderingSiteMapEvent $event): void
    {
        if ($key = $event->key) {
            ...
            if (preg_match('/^solution-sposts-((?:19|20|21|22)\d{2})-(0?[1-9]|1[012])$/', $key, $matches)) {
        ...
        if ($sposts->isNotEmpty()) {
            foreach ($sposts as $post) {
                $key = sprintf('solution-sposts-%s-%s', $post->updated_year, str_pad($post->updated_month, 2, '0', STR_PAD_LEFT));
        ...
        if ($categoryLastUpdated) {
            SiteMapManager::addSitemap(SiteMapManager::route('solution-scategories'), $categoryLastUpdated);
        }
        ...
        if ($tagLastUpdated) {
            SiteMapManager::addSitemap(SiteMapManager::route('solution-stags'), $tagLastUpdated);
        }
    }
}
