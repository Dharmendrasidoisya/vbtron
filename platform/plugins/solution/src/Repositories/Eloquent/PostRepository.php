...
class PostRepository extends RepositoriesAbstract implements PostInterface
{
    ...
    public function getRelated(int|string $id, int $limit = 3): Collection
    {
        ...
        $data = $model
            ...
            ->with('slugable')
            ->orderByDesc('created_at')
            ->whereHas('scategories', function (Builder $query) use ($id) {
                $query->whereIn('scategories.id', $this->getRelatedCategoryIds($id));
            });
    ...
    public function getRelatedCategoryIds(Post|int|string $model): array
    {
        ...
        try {
            return $model->scategories()->allRelatedIds()->toArray();
        } catch (Exception) {
            return [];
        }
    }
    ...
    public function getByCategory(
        ...
        $data = $this->model
            ->wherePublished()
            ->whereHas('scategories', function (Builder $query) use ($categoryId) {
                $query->whereIn('scategories.id', array_filter((array)$categoryId));
            })
            ->select('*')
            ->distinct()
            ->with('slugable')
    ...
    {
        $data = $this->model
            ->with(['slugable', 'scategories', 'scategories.slugable', 'author'])
            ->wherePublished()
            ->whereHas('stags', function (Builder $query) use ($stag) {
                $query->where('stags.id', $stag);
            })
            ->orderByDesc('created_at');
    ...
    public function getRecentsposts(int $limit = 5, int|string $categoryId = 0): Collection
    {
        ...
        if ($categoryId != 0) {
            $data = $data
                ->whereHas('scategories', function (Builder $query) use ($categoryId) {
                    $query->where('scategories.id', $categoryId);
                });
        }
    ...
    public function getPopularsposts(int $limit, array $args = []): Collection
    {
        $data = $this->model
            ->with('slugable')
            ->orderByDesc('views')
    ...
    public function getFilters(array $filters): Collection|LengthAwarePaginator
    {
        ...
        if ($filters['scategories'] !== null) {
            $scategories = array_filter((array)$filters['scategories']);

            $data = $data->whereHas('scategories', function (Builder $query) use ($scategories) {
                $query->whereIn('scategories.id', $scategories);
            });
        }

        if ($filters['scategories_exclude'] !== null) {
            $data = $data
                ->whereHas('scategories', function (Builder $query) use ($filters) {
                    $query->whereNotIn('scategories.id', array_filter((array)$filters['scategories_exclude']));
                });
        }
