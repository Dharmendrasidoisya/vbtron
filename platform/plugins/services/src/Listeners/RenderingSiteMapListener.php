...
namespace Botble\Services\Listeners;
...
class RenderingSiteMapListener
{
    public function handle(RenderingSiteMapEvent $event): void
    {
        if ($key = $event->key) {
            switch ($key) {
                case 'services-servicescategories':
                    ...
                    break;
                case 'services-servicestags':
                    $servicestags = Tag::query()
                        ->with('slugable')
                        ->wherePublished()
                        ->orderByDesc('created_at')
                        ->select(['id', 'name', 'updated_at'])
            ...
            if (preg_match('/^services-servicesposts-((?:19|20|21|22)\d{2})-(0?[1-9]|1[012])$/', $key, $matches)) {
        ...
        if ($servicesposts->isNotEmpty()) {
            foreach ($servicesposts as $post) {
                $key = sprintf('services-servicesposts-%s-%s', $post->updated_year, str_pad($post->updated_month, 2, '0', STR_PAD_LEFT));
        ...
        if ($categoryLastUpdated) {
            SiteMapManager::addSitemap(SiteMapManager::route('services-servicescategories'), $categoryLastUpdated);
        }
        ...
        if ($tagLastUpdated) {
            SiteMapManager::addSitemap(SiteMapManager::route('services-servicestags'), $tagLastUpdated);
        }
    }
}
