...
class PostRepository extends RepositoriesAbstract implements PostInterface
{
    ...
    public function getRelated(int|string $id, int $limit = 3): Collection
    {
        ...
        $data = $model
            ...
            ->with('slugable')
            ->orderByDesc('created_at')
            ->whereHas('servicescategories', function (Builder $query) use ($id) {
                $query->whereIn('servicescategories.id', $this->getRelatedCategoryIds($id));
            });
    ...
    public function getRelatedCategoryIds(Post|int|string $model): array
    {
        ...
        try {
            return $model->servicescategories()->allRelatedIds()->toArray();
        } catch (Exception) {
            return [];
        }
    }
    ...
    public function getByCategory(
        ...
        $data = $this->model
            ->wherePublished()
            ->whereHas('servicescategories', function (Builder $query) use ($categoryId) {
                $query->whereIn('servicescategories.id', array_filter((array)$categoryId));
            })
            ->select('*')
            ->distinct()
            ->with('slugable')
    ...
    {
        $data = $this->model
            ->with(['slugable', 'servicescategories', 'servicescategories.slugable', 'author'])
            ->wherePublished()
            ->whereHas('servicestags', function (Builder $query) use ($stag) {
                $query->where('servicestags.id', $stag);
            })
            ->orderByDesc('created_at');
    ...
    public function getRecentservicesposts(int $limit = 5, int|string $categoryId = 0): Collection
    {
        ...
        if ($categoryId != 0) {
            $data = $data
                ->whereHas('servicescategories', function (Builder $query) use ($categoryId) {
                    $query->where('servicescategories.id', $categoryId);
                });
        }
    ...
    public function getAllservicesposts(
        int $perPage = 12,
        bool $active = true,
        array $with = ['slugable']
    ): Collection|LengthAwarePaginator {
    ...
    public function getPopularservicesposts(int $limit, array $args = []): Collection
    {
        $data = $this->model
            ->with('slugable')
            ->orderByDesc('views')
    ...
    public function getFilters(array $filters): Collection|LengthAwarePaginator
    {
        ...
        if ($filters['servicescategories'] !== null) {
            $servicescategories = array_filter((array)$filters['servicescategories']);

            $data = $data->whereHas('servicescategories', function (Builder $query) use ($servicescategories) {
                $query->whereIn('servicescategories.id', $servicescategories);
            });
        }

        if ($filters['servicescategories_exclude'] !== null) {
            $data = $data
                ->whereHas('servicescategories', function (Builder $query) use ($filters) {
                    $query->whereNotIn('servicescategories.id', array_filter((array)$filters['servicescategories_exclude']));
                });
        }
