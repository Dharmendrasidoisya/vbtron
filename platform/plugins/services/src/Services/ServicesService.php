    ...
    public function handleFrontRoutes(Slug|array $slug): Slug|array|Builder
    {
        ...
        switch ($slug->reference_type) {
            case Post::class:
                ...
                $post = Post::query()
                    ->where($condition)
                    ->with(['servicescategories', 'servicestags', 'slugable', 'servicescategories.slugable', 'servicestags.slugable'])
                    ->firstOrFail();
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/services::posts.edit_this_post'),
                        route('servicesposts.edit', $post->getKey()),
                        null,
                        'servicesposts.edit'
                    );
                }
                ...
                if (function_exists('shortcode')) {
                    shortcode()->getCompiler()->setEditLink(route('servicesposts.edit', $post->id), 'servicesposts.edit');
                }

                $category = $post->servicescategories->sortByDesc('id')->first();
                if ($category) {
                ...
                return [
                    'view' => 'post',
                    'default_view' => 'plugins/services::themes.post',
                    'data' => compact('post'),
                    'slug' => $post->slug,
                ];
            case Category::class:
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/services::categories.edit_this_category'),
                        route('servicescategories.edit', $category->getKey()),
                        null,
                        'servicescategories.edit'
                    );
                }

                $allRelatedCategoryIds = array_merge([$category->getKey()], $category->activeChildren->pluck('id')->all());

                $servicesposts = app(PostInterface::class)
                    ->getByCategory($allRelatedCategoryIds, (int)theme_option('number_of_servicesposts_in_a_category', 12));
                ...
                return [
                    'view' => 'category',
                    'default_view' => 'plugins/services::themes.category',
                    'data' => compact('category', 'servicesposts'),
                    'slug' => $category->slug,
                ];
            case Tag::class:
                $stag = Tag::query()
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/services::tags.edit_this_tag'),
                        route('servicestags.edit', $stag->getKey()),
                        null,
                        'servicestags.edit'
                    );
                }

                $servicesposts = get_servicesposts_by_tag($stag->getKey(), (int)theme_option('number_of_servicesposts_in_a_tag', 12));
                ...
                return [
                    'view' => 'stag',
                    'default_view' => 'plugins/services::themes.stag',
                    'data' => compact('stag', 'servicesposts'),
                    'slug' => $stag->slug,
                ];
        }
