...
use Botble\Services\Repositories\Eloquent\CategoryRepository;
use Botble\Services\Repositories\Eloquent\PostRepository;
use Botble\Services\Repositories\Eloquent\TagRepository;
...
class ServicesServiceProvider extends ServiceProvider
{
    ...
    {
        SlugHelper::registerModule(Post::class, 'Services posts');
        SlugHelper::registerModule(Category::class, 'Services categories');
        SlugHelper::registerModule(Tag::class, 'Services servicestags');
        ...
        $this->app['events']->listen(ThemeRoutingBeforeEvent::class, function () {
            SiteMapManager::registerKey([
                ...
                'services-posts-((?:19|20|21|22)\d{2})-(0?[1-9]|1[012])',
            ]);
        });
        ...
        DashboardMenu::default()->beforeRetrieving(function () {
            DashboardMenu::make()
                ->registerItem([
                    ...
                    'priority' => 5,
                    'name' => 'plugins/services::base.menu_name',
                    'icon' => 'ti ti-article',
                ])
                ->registerItem([
                    'id' => 'cms-plugins-services-post',
                    'priority' => 1,
                    'parent_id' => 'cms-plugins-services',
                    'name' => 'Posts',
                    'route' => 'servicesposts.index',
                ])
                ->registerItem([
                    'id' => 'cms-plugins-services-servicescategories',
                    'priority' => 2,
                    'parent_id' => 'cms-plugins-services',
                    'name' => 'Categories',
                    'route' => 'servicescategories.index',
                ])
                ->registerItem([
                    'id' => 'cms-plugins-services-servicestags',
                    'priority' => 3,
                    'parent_id' => 'cms-plugins-services',
                    'name' => 'Tag',
                    'route' => 'servicestags.index',
                ]);
        });
        ...
        PanelSectionManager::default()->beforeRendering(function () {
            PanelSectionManager::registerItem(
                ...
                fn () => PanelSectionItem::make('services')
                    ->setTitle(trans('plugins/services::base.settings.title'))
                    ->withIcon('ti ti-file-settings')
                    ->withDescription(trans('plugins/services::base.settings.description'))
                    ->withPriority(120)
                    ->withRoute('services.settings')
            );
        });
        ...
        if (defined('LANGUAGE_MODULE_SCREEN_NAME')) {
            if (
                ...
                $this->app['config']->get('plugins.services.general.use_language_v2')
            ) {
        ...
        if (function_exists('shortcode')) {
            view()->composer([
                'plugins/services::themes.post',
                'plugins/services::themes.category',
                'plugins/services::themes.stag',
            ], function (View $view) {
                $view->withShortcodes();
            });
        }
