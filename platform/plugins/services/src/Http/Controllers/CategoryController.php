...
use Botble\Services\Forms\CategoryForm;
use Botble\Services\Http\Requests\CategoryRequest;
...
class CategoryController extends BaseController
{
    protected function breadcrumb(): Breadcrumb
    {
        return parent::breadcrumb()
            ...
            ->add(trans('plugins/services::categories.menu'), route('servicescategories.index'));
    }

    public function index(Request $request)
    {
        $this->pageTitle(trans('plugins/services::categories.menu'));
    ...
    {
        $this->pageTitle(trans('plugins/services::categories.create'));
    ...
    public function store(CategoryRequest $request)
    {
        ...
        return $response
            ->setPreviousRoute('servicescategories.index')
            ->setNextRoute('servicescategories.edit', $category->getKey())
    ...
    {
        if (! $model) {
            $form->setUrl(route('servicescategories.create'));
        }

        if (! Auth::guard()->user()->hasPermission('servicescategories.create') && ! $model) {
    ...
    {
        return array_merge([
            'canCreate' => Auth::guard()->user()->hasPermission('servicescategories.create'),
            'canEdit' => Auth::guard()->user()->hasPermission('servicescategories.edit'),
            'canDelete' => Auth::guard()->user()->hasPermission('servicescategories.destroy'),
            'createRoute' => 'servicescategories.create',
            'editRoute' => 'servicescategories.edit',
            'deleteRoute' => 'servicescategories.destroy',
            'updateTreeRoute' => 'servicescategories.update-tree',
        ], $options);
    }
}
