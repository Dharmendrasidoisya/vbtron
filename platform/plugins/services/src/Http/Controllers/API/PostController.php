...
use Botble\Services\Http\Resources\ListPostResource;
use Botble\Services\Http\Resources\PostResource;
use Botble\Services\Models\Post;
use Botble\Services\Repositories\Interfaces\PostInterface;
use Botble\Services\Supports\FilterPost;
...
class PostController extends BaseController
{
    ...
    public function index(Request $request)
    {
        $data = $this->postRepository
            ->advancedGet([
                'with' => ['servicestags', 'servicescategories', 'author', 'slugable'],
    ...
    public function getSearch(Request $request, PostInterface $postRepository)
    {
        ...
        $servicesposts = $postRepository->getSearch($query);

        $data = [
            'items' => $servicesposts,
            'query' => $query,
            'count' => $servicesposts->count(),
        ];
     ...
     * @queryParam after                Limit response to servicesposts published after a given ISO8601 compliant date.
     * @queryParam author               Limit result set to servicesposts assigned to specific authors.
     * @queryParam author_exclude       Ensure result set excludes servicesposts assigned to specific authors.
     * @queryParam before               Limit response to servicesposts published before a given ISO8601 compliant date.
     ...
     * @queryParam servicescategories           Limit result set to all items that have the specified term assigned in the servicescategories taxonomy.
     * @queryParam servicescategories_exclude   Limit result set to all items except those that have the specified term assigned in the servicescategories taxonomy.
     * @queryParam servicestags                 Limit result set to all items that have the specified term assigned in the servicestags taxonomy.
     * @queryParam servicestags_exclude         Limit result set to all items except those that have the specified term assigned in the servicestags taxonomy.
