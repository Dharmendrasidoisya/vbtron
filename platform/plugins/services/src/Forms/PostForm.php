        ...
        $this
                    ...
                    $form
                        ->add(
                            ...
                            RadioFieldOption::make()
                                ->label(trans('plugins/services::posts.form.format_type'))
                                ->choices($choices)
                                ->toArray()
                        );
                }
            ...
            ->add(
                ...
                SelectFieldOption::make()
                    ->label(trans('plugins/services::posts.form.servicescategories'))
                    ->choices(get_servicescategories_with_children())
                    ->when($this->getModel()->id, function (SelectFieldOption $fieldOption) {
                        return $fieldOption->selected($this->getModel()->servicescategories()->pluck('category_id')->all());
                    })
                    ->when(! $this->getModel()->id, function (SelectFieldOption $fieldOption) {
                        return $fieldOption
            ...
            ->add(
                ...
                TagField::class,
                TagFieldOption::make()
                    ->label(trans('plugins/services::posts.form.servicestags'))
                    ...
                    ->placeholder(trans('plugins/services::base.write_some_servicestags'))
                    ->ajaxUrl(route('servicestags.all'))
                    ->toArray()
            )
