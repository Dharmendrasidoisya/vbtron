    ...
    public function handleFrontRoutes(Slug|array $slug): Slug|array|Builder
    {
        ...
        switch ($slug->reference_type) {
            case Post::class:
                ...
                $post = Post::query()
                    ->where($condition)
                    ->with(['applicationcategories', 'applicationtags', 'slugable', 'applicationcategories.slugable', 'applicationtags.slugable'])
                    ->firstOrFail();
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/application::posts.edit_this_post'),
                        route('applicationposts.edit', $post->getKey()),
                        null,
                        'applicationposts.edit'
                    );
                }

                if (function_exists('shortcode')) {
                    shortcode()->getCompiler()->setEditLink(route('applicationposts.edit', $post->id), 'applicationposts.edit');
                }

                $category = $post->applicationcategories->sortByDesc('id')->first();
                if ($category) {
                ...
                return [
                    'view' => 'post',
                    'default_view' => 'plugins/application::themes.post',
                    'data' => compact('post'),
                    'slug' => $post->slug,
                ];
            case Category::class:
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/application::categories.edit_this_category'),
                        route('applicationcategories.edit', $category->getKey()),
                        null,
                        'applicationcategories.edit'
                    );
                }

                $allRelatedCategoryIds = array_merge([$category->getKey()], $category->activeChildren->pluck('id')->all());

                $applicationposts = app(PostInterface::class)
                    ->getByCategory($allRelatedCategoryIds, (int)theme_option('number_of_applicationposts_in_a_category', 12));
                ...
                return [
                    'view' => 'category',
                    'default_view' => 'plugins/application::themes.category',
                    'data' => compact('category', 'applicationposts'),
                    'slug' => $category->slug,
                ];
            case Tag::class:
                $stag = Tag::query()
                ...
                if (function_exists('admin_bar')) {
                    AdminBar::registerLink(
                        trans('plugins/application::tags.edit_this_tag'),
                        route('applicationtags.edit', $stag->getKey()),
                        null,
                        'applicationtags.edit'
                    );
                }

                $applicationposts = get_applicationposts_by_tag($stag->getKey(), (int)theme_option('number_of_applicationposts_in_a_tag', 12));
                ...
                return [
                    'view' => 'stag',
                    'default_view' => 'plugins/application::themes.stag',
                    'data' => compact('stag', 'applicationposts'),
                    'slug' => $stag->slug,
                ];
        }
