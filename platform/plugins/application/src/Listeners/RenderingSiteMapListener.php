...
namespace Botble\Application\Listeners;
...
class RenderingSiteMapListener
{
    public function handle(RenderingSiteMapEvent $event): void
    {
        if ($key = $event->key) {
            switch ($key) {
                case 'application-applicationcategories':
                    ...
                    break;
                case 'application-applicationtags':
                    $applicationtags = Tag::query()
                        ->with('slugable')
                        ->wherePublished()
                        ->orderByDesc('created_at')
                        ->select(['id', 'name', 'updated_at'])
                    ...
                    foreach ($applicationtags as $stag) {
            ...
            if (preg_match('/^application-applicationposts-((?:19|20|21|22)\d{2})-(0?[1-9]|1[012])$/', $key, $matches)) {
        ...
        if ($applicationposts->isNotEmpty()) {
            foreach ($applicationposts as $post) {
                $key = sprintf('application-applicationposts-%s-%s', $post->updated_year, str_pad($post->updated_month, 2, '0', STR_PAD_LEFT));
        ...
        if ($categoryLastUpdated) {
            SiteMapManager::addSitemap(SiteMapManager::route('application-applicationcategories'), $categoryLastUpdated);
        }
        ...
        if ($tagLastUpdated) {
            SiteMapManager::addSitemap(SiteMapManager::route('application-applicationtags'), $tagLastUpdated);
        }
    }
}
