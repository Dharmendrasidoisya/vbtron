...
class PostRepository extends RepositoriesAbstract implements PostInterface
{
    ...
    public function getRelated(int|string $id, int $limit = 3): Collection
    {
        ...
        $data = $model
            ...
            ->with('slugable')
            ->orderByDesc('created_at')
            ->whereHas('applicationcategories', function (Builder $query) use ($id) {
                $query->whereIn('applicationcategories.id', $this->getRelatedCategoryIds($id));
            });
    ...
    public function getRelatedCategoryIds(Post|int|string $model): array
    {
        ...
        try {
            return $model->applicationcategories()->allRelatedIds()->toArray();
        } catch (Exception) {
            return [];
        }
    }
    ...
    public function getByCategory(
        ...
        $data = $this->model
            ->wherePublished()
            ->whereHas('applicationcategories', function (Builder $query) use ($categoryId) {
                $query->whereIn('applicationcategories.id', array_filter((array)$categoryId));
            })
            ->select('*')
            ->distinct()
            ->with('slugable')
    ...
    public function getByTag(int|string $stag, int $paginate = 12): Collection|LengthAwarePaginator
    {
        $data = $this->model
            ->with(['slugable', 'applicationcategories', 'applicationcategories.slugable', 'author'])
            ->wherePublished()
            ->whereHas('applicationtags', function (Builder $query) use ($stag) {
                $query->where('applicationtags.id', $stag);
            })
            ->orderByDesc('created_at');
    ...
    public function getRecentapplicationposts(int $limit = 5, int|string $categoryId = 0): Collection
    {
        ...
        if ($categoryId != 0) {
            $data = $data
                ->whereHas('applicationcategories', function (Builder $query) use ($categoryId) {
                    $query->where('applicationcategories.id', $categoryId);
                });
        }
    ...
    public function getAllapplicationposts(
        int $perPage = 12,
        bool $active = true,
        array $with = ['slugable']
    ): Collection|LengthAwarePaginator {
    ...
    public function getPopularapplicationposts(int $limit, array $args = []): Collection
    {
        $data = $this->model
            ->with('slugable')
            ->orderByDesc('views')
    ...
    public function getFilters(array $filters): Collection|LengthAwarePaginator
    {
        ...
        if ($filters['applicationcategories'] !== null) {
            $applicationcategories = array_filter((array)$filters['applicationcategories']);

            $data = $data->whereHas('applicationcategories', function (Builder $query) use ($applicationcategories) {
                $query->whereIn('applicationcategories.id', $applicationcategories);
            });
        }

        if ($filters['applicationcategories_exclude'] !== null) {
            $data = $data
                ->whereHas('applicationcategories', function (Builder $query) use ($filters) {
                    $query->whereNotIn('applicationcategories.id', array_filter((array)$filters['applicationcategories_exclude']));
                });
        }
