...
use Botble\Application\Forms\CategoryForm;
use Botble\Application\Http\Requests\CategoryRequest;
...
class CategoryController extends BaseController
{
    protected function breadcrumb(): Breadcrumb
    {
        return parent::breadcrumb()
            ...
            ->add(trans('plugins/application::categories.menu'), route('applicationcategories.index'));
    }

    public function index(Request $request)
    {
        $this->pageTitle(trans('plugins/application::categories.menu'));
    ...
    {
        $this->pageTitle(trans('plugins/application::categories.create'));
    ...
    public function store(CategoryRequest $request)
    {
        ...
        return $response
            ->setPreviousRoute('applicationcategories.index')
            ->setNextRoute('applicationcategories.edit', $category->getKey())
    ...
    {
        if (! $model) {
            $form->setUrl(route('applicationcategories.create'));
        }

        if (! Auth::guard()->user()->hasPermission('applicationcategories.create') && ! $model) {
    ...
    {
        return array_merge([
            'canCreate' => Auth::guard()->user()->hasPermission('applicationcategories.create'),
            'canEdit' => Auth::guard()->user()->hasPermission('applicationcategories.edit'),
            'canDelete' => Auth::guard()->user()->hasPermission('applicationcategories.destroy'),
            'createRoute' => 'applicationcategories.create',
            'editRoute' => 'applicationcategories.edit',
            'deleteRoute' => 'applicationcategories.destroy',
            'updateTreeRoute' => 'applicationcategories.update-tree',
        ], $options);
    }
}
