...
class PublicController extends BaseController
{
    public function getSearch(Request $request, PostInterface $postRepository)
    {
        ...
        $applicationposts = $postRepository->getSearch($query, 0, (int)theme_option('number_of_applicationposts_in_a_category', 12));

        Theme::breadcrumb()->add($title, route('public.search'));

        return Theme::scope('search', compact('applicationposts'))
            ->render();
    }
}
