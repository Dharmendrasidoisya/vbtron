...
class PostRepository extends RepositoriesAbstract implements PostInterface
{
    ...
    public function getRelated(int|string $id, int $limit = 3): Collection
    {
        ...
        $data = $model
            ...
            ->with('slugable')
            ->orderByDesc('created_at')
            ->whereHas('productscategories', function (Builder $query) use ($id) {
                $query->whereIn('productscategories.id', $this->getRelatedCategoryIds($id));
            });
    ...
    public function getRelatedCategoryIds(Post|int|string $model): array
    {
        ...
        try {
            return $model->productscategories()->allRelatedIds()->toArray();
        } catch (Exception) {
            return [];
        }
    }
    ...
    public function getByCategory(
        ...
        $data = $this->model
            ->wherePublished()
            ->whereHas('productscategories', function (Builder $query) use ($categoryId) {
                $query->whereIn('productscategories.id', array_filter((array)$categoryId));
            })
            ->select('*')
            ->distinct()
            ->with('slugable')
    ...
    {
        $data = $this->model
            ->with(['slugable', 'productscategories', 'productscategories.slugable', 'author'])
            ->wherePublished()
            ->whereHas('productstags', function (Builder $query) use ($stag) {
                $query->where('productstags.id', $stag);
            })
            ->orderByDesc('created_at');
    ...
    public function getRecentproductsposts(int $limit = 5, int|string $categoryId = 0): Collection
    {
        ...
        if ($categoryId != 0) {
            $data = $data
                ->whereHas('productscategories', function (Builder $query) use ($categoryId) {
                    $query->where('productscategories.id', $categoryId);
                });
        }
    ...
    public function getAllproductsposts(
        int $perPage = 12,
        bool $active = true,
        array $with = ['slugable']
    ): Collection|LengthAwarePaginator {
    ...
    public function getPopularproductsposts(int $limit, array $args = []): Collection
    {
        $data = $this->model
            ->with('slugable')
            ->orderByDesc('views')
    ...
    public function getFilters(array $filters): Collection|LengthAwarePaginator
    {
        ...
        if ($filters['productscategories'] !== null) {
            $productscategories = array_filter((array)$filters['productscategories']);

            $data = $data->whereHas('productscategories', function (Builder $query) use ($productscategories) {
                $query->whereIn('productscategories.id', $productscategories);
            });
        }

        if ($filters['productscategories_exclude'] !== null) {
            $data = $data
                ->whereHas('productscategories', function (Builder $query) use ($filters) {
                    $query->whereNotIn('productscategories.id', array_filter((array)$filters['productscategories_exclude']));
                });
        }
