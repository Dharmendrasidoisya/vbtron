...
namespace Botble\Products\Listeners;
...
class RenderingSiteMapListener
{
    public function handle(RenderingSiteMapEvent $event): void
    {
        if ($key = $event->key) {
            switch ($key) {
                case 'products-productscategories':
                    ...
                    break;
                case 'products-productstags':
                    $productstags = Tag::query()
                        ->with('slugable')
                        ->wherePublished()
                        ->orderByDesc('created_at')
                        ->select(['id', 'name', 'updated_at'])
            ...
            if (preg_match('/^products-productsposts-((?:19|20|21|22)\d{2})-(0?[1-9]|1[012])$/', $key, $matches)) {
        ...
        if ($productsposts->isNotEmpty()) {
            foreach ($productsposts as $post) {
                $key = sprintf('products-productsposts-%s-%s', $post->updated_year, str_pad($post->updated_month, 2, '0', STR_PAD_LEFT));
        ...
        if ($categoryLastUpdated) {
            SiteMapManager::addSitemap(SiteMapManager::route('products-productscategories'), $categoryLastUpdated);
        }
        ...
        if ($tagLastUpdated) {
            SiteMapManager::addSitemap(SiteMapManager::route('products-productstags'), $tagLastUpdated);
        }
    }
}
