                            ...
                            <div class="col" style="text-align:center;">
                                ...
                                <div class="appear-animation animated fadeIn appear-animation-visible"
                                    ...
                                    <ul class="breadcrumb d-flex text-3-5 font-weight-semi-bold pb-2 mb-3">
                                        ...
                                        <li class="active text-color-light opacity-7">{{ $product->name }}
                                        </li>
                                    </ul>
                                </div>
                            </div>
    ...
    <div class="row">
        <div class="col-lg-4 order-1 order-lg-0 pe-lg-5 mt-4 mt-lg-0">
            ...
            <div class="bg-grey-100 p-4 border-radius-2 mb-4">
                <div class="m-3">
                    <h4 class="text-5 font-weight-semibold line-height-1 mb-4">{{__('Related products')}}</h4>
                    <ul class="nav nav-list nav-list-arrows flex-column mb-0">
                        @foreach (get_related_products($product) as $relatedProduct)
                            <li class="nav-item">
                                <a href="{{ $relatedProduct->url }}" class="nav-link ">{{ $relatedProduct->name }}</a>
                            </li>
                        @endforeach
                    </ul>
            ...
            <div class="bg-tertiary text-light p-4 border-radius-2 mb-4">
                <div class="m-3">
                    <h4 class="text-5 font-weight-semibold line-height-1 mb-4">{{ $product->name }}</h4>
                    <div>
        ...
        <div class="col-lg-8">
            @foreach ($productImages as $img)
                <img src="{{ RvMedia::getImageUrl($img, 'thumb') }}" loading="lazy" class="img-fluid"
                    alt="{{ $product->name }}">
            @endforeach
...
{{-- <div class="product-detail accordion-detail">
            ...
            <div class="detail-info">
                ...
                <div class="product-detail-rating">
                    @if ($product->brand->id)
                        <div class="pro-details-brand">
                            <span class="d-inline-block me-1">{{ __('Brands') }}:</span> <a
                    ...
                    @if (EcommerceHelper::isReviewEnabled())
                        <div class="product-rate-cover text-end">
                            <div class="rating_wrap">
                                ...
                                <span
                                    class="rating_num">({{ __(':count reviews', ['count' => $product->reviews_count]) }})</span>
                            </div>
                        </div>
                    @endif
                </div>
                <div class="clearfix product-price-cover">
                    <div class="product-price primary-color float-left">
                        <ins><span
                                class="text-brand">{{ format_price($product->front_sale_price_with_taxes) }}</span></ins>
                        @if ($product->front_sale_price !== $product->price)
                            ...
                            <span class="save-price font-md color3 ml-15"><span
                                    class="percentage-off d-inline-block">{{ get_sale_percentage($product->price, $product->front_sale_price) }}</span>
                                <span class="d-inline-block">{{ __('Off') }}</span></span>
                        @endif
                    </div>
                </div>
                ...
                <ul class="product-meta font-xs color-grey mt-50">
                    ...
                    <li class="mb-5 @if (!$product->sku) d-none @endif"><span
                            class="d-inline-block me-1" id="product-sku">{{ __('SKU') }}</span>:
                    ...
                    <li><span class="d-inline-block me-1">{{ __('Availability') }}:</span> <span
                            class="in-stock text-success ml-5">{!! BaseHelper::clean($product->stock_status_html) !!}</span></li>
                </ul>
            </div>
            <!-- Detail Info -->
    ...
    <div class="tab-style3">
        <ul class="nav nav-tabs text-uppercase">
            ...
            @if (EcommerceHelper::isReviewEnabled())
                <li class="nav-item">
                    <a class="nav-link" id="Reviews-tab" data-bs-toggle="tab" href="#Reviews">{{ __('Reviews') }}
